...
final class SecurityActions {
	...
	public static List<Class<?>> getClassWithAnnotation(final Class<?> source, final Class<? extends Annotation> annotationClass) {
		final List<Class<?>> declaredAccessableFields = AccessController.doPrivileged(new PrivilegedAction<List<Class<?>>>() {
			public List<Class<?>> run() {
				List<Class<?>> foundFields = new ArrayList<Class<?>>();
				Class<?> nextSource = source;
				while (nextSource != Object.class) {
					if (nextSource.isAnnotationPresent(annotationClass)) {
						foundFields.add(nextSource);
		            }
